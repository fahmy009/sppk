...
if (isset($_POST['submit'])) {
    ...
    $select_pa = mysqli_query($con, "SELECT * FROM `nilai_kriteria`  WHERE  `jenis` =  'kriteria' AND  `user_id` =  '$user_id'");
    ...
    if ($num_pa > 0) {
        ...
        while ($row_pa = mysqli_fetch_object($select_pa)) {
            $evx = $ev[$i];
            $run_update .= mysqli_query($con, "UPDATE  `nilai_kriteria` SET  `matrik_value` =  '$evx' WHERE  `id_matrik` =  '$row_pa->id_matrik' AND `jenis` =  'kriteria' AND `user_id` = '$user_id'");
            $i++;
        }
    } else {
        for ($i = 0; $i <= 3; $i++) {
            $evx = $ev[$i];
            $run_insert .= mysqli_query($con, "INSERT INTO `nilai_kriteria` (`id_matrik`, `user_id`, `matrik_value`,`jenis`) VALUES ('$i','$user_id', '$evx','kriteria')");
        }
    }
