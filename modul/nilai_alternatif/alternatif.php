...
$user_id =$_SESSION['namauser'];
	...
	$select_pa = mysql_query("SELECT * FROM `nilai_alternatif`  WHERE  `jenis` =  'alternatif' AND  `user_id` =  '$user_id'");
	...
	if( $num_pa > 0 ){	
		...
		$evx = $ev[$i];
		$run_update.= mysql_query("UPDATE  `nilai_alternatif` SET  `matrik_value` =  '$evx' WHERE  `id_matrik` =  '$row_pa->id_matrik' AND `jenis` =  'alternatif' AND `user_id` = '$user_id'");
		$i++;
		}
	}else{
		for($i=0; $i<=11; $i++){
		$evx = $ev[$i];
		$run_insert.= mysql_query("INSERT INTO `nilai_alternatif` (`id_matrik`, `user_id`, `matrik_value`,`jenis`) VALUES ('$i','$user_id', '$evx','alternatif')");
		}
	}
...
$id = get_uname_id();
$query_kriteria = mysql_query("SELECT * FROM nilai_alternatif WHERE jenis='kriteria' AND user_id='$id'");
...
while($row_alternatif = mysql_fetch_object($query_alternatif)){
