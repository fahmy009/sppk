...
function matrik_id($id,$jenis = 'alternatif'){
	...
	$query_alternatif = mysql_query("SELECT * FROM `nilai_alternatif` WHERE jenis='$jenis' AND user_id='$user_id' AND id_matrik='$id'");
...
function list_npm_op($op_link = false){
	if( empty($op_link) ) $link = '?module=kandidat'; 
		...
		$query_nip = mysql_query("SELECT * FROM pegawai");
