...
switch($_GET[act]){
	default:
	$tampil=mysql_query("select * from guru,bagian,jabatan where guru.id_bag=bagian.id_bag and
...
echo "  
	...
	$ambilfoto=mysql_query("select * from guru where nip='$_GET[id]'");
	$t1=mysql_fetch_array($ambilfoto);
	$ambil=mysql_query("select * from kandidat where nip='$_GET[id]'");
	$t=mysql_fetch_array($ambil);
