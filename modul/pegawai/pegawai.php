...
switch ($_GET[act]) {
    default:
        $tampil = mysqli_query($con, "select * from pegawai,bagian,jabatan where pegawai.id_bag=bagian.id_bag and
	pegawai.id_jab=jabatan.id_jab order by nip DESC");
        echo "<h2 class='head'>DATA GURU</h2>
	<div>
	<input type=button value='Tambah Data Guru' onclick=\"window.location.href='?module=pegawai&act=input';\">
	</div>
	<table class='tabel'>
	<thead>
  <tr>
    ...
    case "edit":
        $ambil = mysqli_query($con, "select * from pegawai where nip='$_GET[id]'");
    ...
    // LIhat Nilai Guru 
    case "lihat-nilai":
        $ambil = mysqli_query($con, "select * from kandidat,pegawai where pegawai.nip=kandidat=.'$_GET[id]'");
        $t = mysqli_fetch_array($ambil);
