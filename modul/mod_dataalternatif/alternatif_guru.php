...
function get_kriteria($param)
{
    ...
    $kandidat = array();
    $query_kandidat = mysqli_query($con, "SELECT * FROM kriteria ");
...
$query_nip = mysqli_query($con, "SELECT * FROM guru WHERE nip='" . $_GET['nip'] . "'");
$row_nip = mysqli_fetch_object($query_nip);
...
if (isset($_POST['submit'])) {
    ...
    $query_nip_jumlah = mysqli_query($con, "SELECT * FROM kandidat WHERE user_id='$user_id'");
    $num_nip = mysqli_num_rows($query_nip_jumlah);
    ...
    elseif ($num_nip >= 22) echo '<div class="error">Maaf jumlah kandidat  sudah melebihi batas</div><br>';
        ...
        $query_nip_run = mysqli_query($con, "INSERT INTO kandidat (nip,user_id,nama) VALUES ('$nip','$user_id','$nama')");
    ...
    <table width="100%" border="0" cellspacing="0" cellpadding="2" class="tabel">
        <tr>
            ...
            <td width="78%"><?php echo js_redirect() ?>
                <select name="npm_op" onchange="redir(this)">
                    <option value="">-- Pilih --</option><?php list_npm_op() ?></select>
...
if ($_GET['act'] == 'del') {
    $nip = $_GET['nip'];
    $del_query = mysqli_query($con, "DELETE FROM kandidat WHERE  nip='$nip'");
...
<table width="100%" border="0" cellspacing="0" cellpadding="2" class="tabel">
    ...
    <?php
    $i = 1;
    $query_nip = mysqli_query($con, "SELECT * FROM kandidat WHERE user_id='$user_id'");
    $num_nip = mysqli_num_rows($query_nip);
    ...
    while ($row_nip = mysqli_fetch_object($query_nip)) {
        ...
        <tr>
            ...
            <td align="center">
                <div align="left"><?php echo $row_nip->nip ?></div>
            </td>
            <td align="center">
                <div align="left"><?php echo $row_nip->nama ?></div>
            </td>
            <td align="center"><a
                        href="?module=dataalternatif&amp;act=del&amp;nip=<?php echo $row_nip->nip ?>">Hapus</a></td>
        </tr>
        <?php $i++;
    } ?>
</table>
